  ...
  module CoarNotify
    class RequestReview
      ...
      def notify_success(response)
        notify(
          I18n.t(
            "coar_notify.request_review_notification.success_body",
            work_link: work_link,
            response_body: response.body,
            default: "Review requested successfully for #{work_link}: #{response.body}"
          )
        )
      end
      ...
      def notify_failure(message)
        notify(
          I18n.t(
            "coar_notify.request_review_notification.failure_body",
            work_link: work_link,
            response_body: message,
            default: "Failed to request review for #{work_link}: #{message}"
          )
        )
      end
        ...
        if defined?(Hyrax::MessengerService)
          begin
            Hyrax::MessengerService.deliver(
              ...
              recipients,
              message,
              I18n.t("coar_notify.request_review_notification.subject", default: "COAR Notify: Review Request")
            )
      ...
      def file_set
        @file_set ||= begin
          ...
          raise MissingFileSetError, I18n.t('coar_notify.request_review_notification.fileset_missing', default: "No file set found for work") unless found_fs
