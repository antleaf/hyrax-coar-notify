...
module Hyrax
  module CoarNotify
    # Shared implementation for RequestEndorsement and RequestReview: builds a COAR Notify
    # "Offer" activity, sends it to the target service, records it, and reports success or
    # failure to the user. Subclasses provide REQUEST_TYPE, ACTIVITY_TYPE, I18N_SCOPE and
    # ACTION_NOUN (used only to build the default English text below, when no translation exists).
    class RequestBase
      ...
      def call
        ...
        # Deliberately re-raises after notifying: this also catches things like NoMemoryError, so the
        # user should still hear about it, but the job system (ActiveJob's retry/failure tracking) must
        # find out too. (RequestEndorsement re-raised here before this class existed; RequestReview did
        # not, which was an accidental inconsistency between the two, not a considered difference.)
        notify_failure(e.message)
        raise
      end
      ...
      def notify_success(response)
        notify(
          I18n.t(
            "#{i18n_scope}.success_body",
            work_link: work_link,
            response_body: response.body,
            default: "#{action_noun} requested successfully for #{work_link}: #{response.body}"
          )
        )
      end
      ...
      def notify_failure(message)
        notify(
          I18n.t(
            "#{i18n_scope}.failure_body",
            work_link: work_link,
            response_body: message,
            default: "Failed to request #{action_noun.downcase} for #{work_link}: #{message}"
          )
        )
      end
        ...
        if defined?(Hyrax::MessengerService)
          begin
            Hyrax::MessengerService.deliver(
              ...
              recipients,
              message,
              I18n.t("#{i18n_scope}.subject", default: "COAR Notify: #{action_noun} Request")
            )
      ...
      def ietf_item
...

        {
          ...
          type: %w[Article sorg:ScholarlyArticle]
        }
      end
      ...
      def file_set
        @file_set ||= begin
          ...
          raise MissingFileSetError, I18n.t("#{i18n_scope}.fileset_missing", default: "No file set found for work") unless found_fs
